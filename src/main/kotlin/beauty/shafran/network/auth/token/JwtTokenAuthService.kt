...
class JwtTokenAuthService(
    ...
    override suspend fun generateTokenForSession(session: AccountSessionEntity): TokenData {
        val expiresAt = Date(System.currentTimeMillis() + 30 * 1000)
        return JwtTokenData(
    ...
    private fun extractSessionId(token: String): String {
        return try {
            verifier.verify(token).getClaim("sessionId").asString()
    ...
    override suspend fun extractSessionId(token: AccessTokenData): String {
        when (token) {
    ...
    override suspend fun extractSessionId(token: RefreshTokenData): String {
        when (token) {
